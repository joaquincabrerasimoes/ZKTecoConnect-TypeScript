...
import {
    CMD_CONNECT, CMD_EXIT, CMD_AUTH, CMD_ACK_OK, CMD_ACK_UNAUTH, CMD_ACK_ERROR, CMD_ACK_UNKNOWN,
    MACHINE_PREPARE_DATA_1, MACHINE_PREPARE_DATA_2, USHRT_MAX, CMD_PREPARE_DATA, CMD_DATA,
    CMD_GET_VERSION, CMD_OPTIONS_RRQ, CMD_GET_TIME, CMD_USER_WRQ, CMD_DELETE_USER, CMD_USERTEMP_RRQ,
    CMD_ATTLOG_RRQ, CMD_RESTART, CMD_UNLOCK, CMD_ENABLEDEVICE, CMD_DISABLEDEVICE, CMD_SET_TIME, CMD_TESTVOICE,
    CMD_REG_EVENT, EF_ATTLOG, CMD_READ_BUFFER, CMD_PREPARE_BUFFER, CMD_FREE_DATA, CMD_GET_FREE_SIZES, FCT_USER, CMD_ACK_DATA,
    CMD_DB_RRQ, FCT_FINGERTMP, CMD_DELETE_USERTEMP, _CMD_GET_USERTEMP, _CMD_DEL_USER_TEMP
} from './constants.js';
...
export class ZKTeco {
    ...
    private forceUdp: boolean;
    private verbose: boolean;

    private socket: net.Socket | dgram.Socket | null = null;
    private isConnected: boolean = false;
    private sessionId: number = 0;
    private replyId: number = USHRT_MAX - 1;
    private tcpLength: number = 0;
    private lastResponse: number = 0;
    private lastData: Buffer = Buffer.alloc(0);

    private users: number = 0;
    private fingers: number = 0;
    private records: number = 0;
    private usersCapacity: number = 0;
    private fingersCapacity: number = 0;
    private recordsCapacity: number = 0;

    constructor(ip: string, port: number = 4370, timeout: number = 10000, password: number = 0, forceUdp: boolean = false, verbose: boolean = false) {
        this.ip = ip;
        this.port = port;
    ...
    private async sendCommand(command: number, commandString: Buffer, responseSize: number): Promise<Buffer | null> {
        if (!this.socket) throw new Error('Socket not initialized');

        this.replyId++;
        if (this.replyId >= USHRT_MAX) this.replyId = 0;
        ...
        if (this.socket instanceof net.Socket) {
            ...
            return new Promise((resolve, reject) => {
                ...
                socket.send(packet, this.port, this.ip, (err: Error | null) => {
                    if (err) {
                        cleanup();
                        reject(err);
                    }
    ...
    public async getUsers(): Promise<ZKTecoUser[]> {
        ...
        if (this.verbose) console.log('Users read:', size);
        if (size <= 4) {
            return [];
        }
        ...
        if (this.verbose) console.log('User packet size:', userPacketSize);
    ...
    private async readSizes(): Promise<void> {
        if (this.verbose) console.log('Reading sizes...');
        const memInfo = await this.getMemoryInfo();
        if (memInfo) {
            if (this.verbose) console.log('Sizes read:', memInfo);
    ...
    private async receiveRawData(size: number): Promise<Buffer> {
        return new Promise((resolve, reject) => {
            ...
            const onData = (data: Buffer) => {
                ...
                if (this.verbose) console.log(`receiveRawData: received ${data.length}, remaining ${remaining}`);
    ...
    private async receiveChunk(): Promise<Buffer> {
        // Handle CMD_DATA response
        if (this.lastResponse === CMD_DATA) {
            if (!this.forceUdp && this.socket instanceof net.Socket) {
                if (this.verbose) console.log(`_rc_DATA! is ${this.lastData.length} bytes, tcp length is ${this.tcpLength}`);
                ...
                // UDP case
                if (this.verbose) console.log(`_rc len is ${this.lastData.length}`);
                return this.lastData;
            }
        }
        // Handle CMD_PREPARE_DATA response
            ...
            if (this.verbose) console.log(`receive chunk: prepare data size is ${size}`);
            ...
            if (!this.forceUdp && this.socket instanceof net.Socket) {
                ...
                while (true) {
                    ...
                    if (this.verbose) console.log(`still needs ${remaining}`);
            ...
            if (this.verbose) console.log(`invalid response ${this.lastResponse}`);
    ...
    private async readChunk(start: number, size: number): Promise<Buffer> {
        for (let retries = 0; retries < 3; retries++) {
            try {
                ...
                if (this.verbose) console.log(`ReadChunk: receiveChunk returned empty data on retry ${retries}`);
            } catch (e) {
                if (this.verbose) console.log(`ReadChunk retry ${retries}:`, e);
            }
        }
        throw new Error(`Can't read chunk ${start}:[${size}]`);
    }

    private async readWithBuffer(command: number, fct: number = 0, ext: number = 0): Promise<{ data: Buffer, size: number }> {
        ...
        if (this.lastResponse === CMD_DATA) {
            if (this.socket instanceof net.Socket && !this.forceUdp) {
                ...
                if (this.verbose) console.log(`DATA! is ${this.lastData.length} bytes, tcp length is ${this.tcpLength}`);
        ...
        if (this.verbose) console.log(`size will be ${dataSize}`);
        ...
        if (this.verbose) console.log(`rwb: #${packets} packets of max ${MAX_CHUNK} bytes, and extra ${remain} bytes remain`);
    ...
    public async setUser(uid: number, name: string, password: string, role: number = 0, card: number = 0): Promise<boolean> {
