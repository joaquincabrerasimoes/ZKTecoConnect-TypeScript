...
export async function sendAckOnly(client, receivedHeader = null) {
    if (!client.socket)
        return;
...
export async function receiveLivePacket(client, timeoutMs = 1000) {
    if (!client.socket)
        throw new Error('Socket not initialized');
...
export async function flushExistingEvents(client, options = {}) {
...
export function processLiveEventBuffer(buffer, userLookup, verbose = false) {
    const events = [];
