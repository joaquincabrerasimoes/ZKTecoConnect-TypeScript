...
exports.readWithBuffer = readWithBuffer;
exports.receiveRawData = receiveRawData;
exports.getDataSize = getDataSize;
exports.receiveChunk = receiveChunk;
exports.readChunk = readChunk;
exports.freeData = freeData;
exports.sendAckOnly = sendAckOnly;
exports.receiveLivePacket = receiveLivePacket;
exports.flushExistingEvents = flushExistingEvents;
exports.processLiveEventBuffer = processLiveEventBuffer;
const net = __importStar(require("net"));
const dgram = __importStar(require("dgram"));
const constants_1 = require("../others/constants");
const utils_1 = require("../utils/utils");
const zkTecoAttendance_1 = require("../objects/zkTecoAttendance");
async function createSocket(ip, port, timeout, forceUdp, client) {
...
async function sendCommand(command, commandString, responseSize, client) {
    if (!client.socket)
        throw new Error('Socket not initialized');
    client.replyId++;
    if (client.replyId >= constants_1.USHRT_MAX)
        client.replyId = 0;
    const packet = (0, utils_1.createHeader)(command, commandString, client.sessionId, client.replyId);
    if (client.socket instanceof net.Socket) {
        const tcpPacket = (0, utils_1.createTcpTop)(packet);
        return new Promise((resolve, reject) => {
            let timeoutId;
            const cleanup = () => {
            ...
            const onData = (data) => {
                if (data.length >= 8) {
                    ...
                    if (header1 === constants_1.MACHINE_PREPARE_DATA_1 && header2 === constants_1.MACHINE_PREPARE_DATA_2) {
...
async function readSizes(client) {
    if (client.verbose)
...
async function readWithBuffer(command, fct = 0, ext = 0, client) {
    ...
    const response = await sendCommand(constants_1.CMD_PREPARE_BUFFER, commandString, 1024, client);
    if (!response) {
    ...
    if (client.lastResponse === constants_1.CMD_DATA) {
...
async function receiveRawData(size, client) {
    return new Promise((resolve, reject) => {
...
function getDataSize(client) {
    if (client.lastResponse === constants_1.CMD_PREPARE_DATA) {
...
async function receiveChunk(client) {
    ...
    // Handle CMD_PREPARE_DATA response
    else if (client.lastResponse === constants_1.CMD_PREPARE_DATA) {
        ...
        if (!client.forceUdp && client.socket instanceof net.Socket) {
            ...
            while (true) {
                ...
                if (response === constants_1.CMD_DATA) {
                    data.push(dataRecv.subarray(8));
                    remaining -= 1024;
                }
                else if (response === constants_1.CMD_ACK_OK) {
                    break;
                }
                else {
                    if (client.verbose)
...
async function readChunk(start, size, client) {
    for (let retries = 0; retries < 3; retries++) {
        try {
            ...
            await sendCommand(constants_1.CMD_READ_BUFFER, commandString, responseSize, client);
...
async function freeData(client) {
    await sendCommand(constants_1.CMD_FREE_DATA, Buffer.alloc(0), 1024, client);
}
async function sendAckOnly(client, receivedHeader = null) {
    if (!client.socket)
        return;
    let ackSessionId = client.sessionId;
    let ackReplyId = constants_1.USHRT_MAX - 1;
    ...
    const packet = (0, utils_1.createHeader)(constants_1.CMD_ACK_OK, Buffer.alloc(0), ackSessionId, ackReplyId);
...
async function receiveLivePacket(client, timeoutMs = 1000) {
    if (!client.socket)
        throw new Error('Socket not initialized');
    const cleanupTcp = (socket, handlers, timer) => {
...
async function flushExistingEvents(client, options = {}) {
    const { timeoutMs = 200, maxPackets = 10, verbose = false } = options;
    let flushed = 0;
...
function processLiveEventBuffer(buffer, userLookup, verbose = false) {
    const events = [];
...
function extractLivePacket(packet, headerOffset) {
...
function consumePendingTcpPacket(client) {
...
function determineEventSize(bufferLength) {
...
function parseLiveEventData(data, userLookup, verbose) {
    let userId = '';
    let status = 0;
    let punch = 0;
    let timeBytes = null;
    if (data.length === 10 || data.length === 14) {
        ...
        userId = (0, utils_1.removeNull)(data.subarray(0, 24).toString());
    ...
    return new zkTecoAttendance_1.ZKTecoAttendance(userId, uid, timestamp, status, punch);
}
