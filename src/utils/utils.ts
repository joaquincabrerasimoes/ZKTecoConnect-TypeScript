...
export const createHeader = (command: number, commandString: Buffer, sessionId: number, replyId: number): Buffer => {
...
export const createTcpTop = (packet: Buffer): Buffer => {
...
export const testTcpTop = (packet: Buffer): number => {
...
export const makeCommKey = (key: number, sessionId: number, ticks: number = 50): Buffer => {
    let k = 0;
    for (let i = 0; i < 32; i++) {
        if ((key & (1 << i))) {
            k = (k << 1) | 1;
...
export const decodeTime = (timeBytes: Buffer): Date => {
...
export const encodeTime = (date: Date): number => {
    return (
        ((date.getFullYear() % 100) * 12 * 31 + ((date.getMonth() + 1) - 1) * 31 + date.getDate() - 1) *
        (24 * 60 * 60) +
...
export const removeNull = (str: string): string => {
